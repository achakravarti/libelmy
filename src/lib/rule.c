                   ...
                   elmy_logs_t **res, elmy_error_t **err)
{
        ...
        const char *rule = "hostname";
        const char *sql = "SELECT * FROM logs_hostname($1,$2);";
        const char *sqlp = "SELECT * FROM logs_hostname_paged"
                           "($1,$2,$3,$4,$5,$6);";
...
int elmy_rule_tag(const char *filter, const char *tz, const elmy_page_t *pg,
                  elmy_logs_t **res, elmy_error_t **err)
{
        assert(filter != NULL && *filter != '\0');
        assert(tz != NULL && *tz != '\0');
        ...
        const char *rule = "hostname";
        const char *sql = "SELECT * FROM logs_tag($1,$2);";
        const char *sqlp = "SELECT * FROM logs_tag_paged"
                           "($1,$2,$3,$4,$5,$6);";
...
int elmy_rule_message(const char *filter, const char *tz, const elmy_page_t *pg,
                      elmy_logs_t **res, elmy_error_t **err)
{
        assert(filter != NULL && *filter != '\0');
        assert(tz != NULL && *tz != '\0');
        ...
        const char *rule = "hostname";
        const char *sql = "SELECT * FROM logs_message($1,$2);";
        const char *sqlp = "SELECT * FROM logs_message_paged"
                           "($1,$2,$3,$4,$5,$6);";
